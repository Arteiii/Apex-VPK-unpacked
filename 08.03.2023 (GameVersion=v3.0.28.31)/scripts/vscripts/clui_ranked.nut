...
bool function Ranked_HasBeenInitialized()
{
	...
	if ( SharedRankedPeriod_HasSplits( activeRankedPeriod ) )
	{
		if ( GetCurrentPlaylistVarBool( "ranked_end_series_stryder_check", true ) )                                                                                                                                                             
		{
			if ( SharedRankedPeriod_IsSecondSplitActive( activeRankedPeriod ) && ( GetRankedPersistenceData( uiPlayer, "endFirstSplitLadderPosition" ) == 0 || GetRankedPersistenceData( uiPlayer, "hasSplitResetOccured" ) == 0 ) )
				return false
		}
	}
