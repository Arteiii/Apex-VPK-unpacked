...
bool function IsLootActionTypeButtonDown( int actionType )
{
	switch( actionType )
	{
		case eLootActionType.ALT_ACTION:
			return InputIsButtonDown( MOUSE_RIGHT ) || InputIsButtonDown( BUTTON_X )
			break

		case eLootActionType.PRIMARY_SWAP:
			return InputIsButtonDown( KEY_V ) || InputIsButtonDown( BUTTON_SHOULDER_LEFT )
			break
			
		case eLootActionType.CHARACTER_ACTION1:
			return InputIsButtonDown( KEY_Q ) || InputIsButtonDown( BUTTON_TRIGGER_LEFT )
			break

		case eLootActionType.CHARACTER_ACTION2:
			return InputIsButtonDown( KEY_E ) || InputIsButtonDown( BUTTON_TRIGGER_RIGHT )
			break
	}
...
void function UpdateBackpackDpadNav()
{
	...
	var navUpButton = Hud_GetChild( file.mainInventoryPanel, "MainWeapon0" )
