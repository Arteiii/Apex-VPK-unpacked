...
const float HEARTBEAT_SENSOR_STARTUP_TARGET_DELAY_MIN = HEARTBEAT_SENSOR_PING_INTERVAL_MIN * 0.6                                                                                           
const float HEARTBEAT_SENSOR_STARTUP_TARGET_DELAY_MAX = HEARTBEAT_SENSOR_PING_INTERVAL_MAX * 0.6
...
void function ManageVictims_Thread( entity player )
{
	...
	while ( true )
	{
			...
			if ( HEARTBEAT_SENSOR_DEBUG_VERBOSE )
			{
				...
				DebugDrawCylinder( player.EyePosition(), < -90, 0, 0 >, file.heartbeatSensorRange, 750, COLOR_RED, true, 0.1 )
			}
		#endif      
