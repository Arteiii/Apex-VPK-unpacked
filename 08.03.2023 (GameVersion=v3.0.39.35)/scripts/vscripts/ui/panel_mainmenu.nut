...
bool function IsAccessibilityFooterValid()
{
	...
	#if DURANGO_PROG
		return Console_IsSignedIn() && !IsWorking() && !IsSearchingForPartyServer() && !IsCrossProgressing()
	#else
		return !IsWorking() && !IsSearchingForPartyServer() && !IsCrossProgressing()
	#endif
}
...
bool function IsDataCenterFooterClickable()
{
	...
	#if DURANGO_PROG
		return Console_IsSignedIn() && !IsWorking() && !IsSearchingForPartyServer() && !IsCrossProgressing() && hideDurationElapsed
	#else
...
#if CONSOLE_PROG
	if( GetConVarInt( "StryderUseClientJwt" ) == 1)
	{
		bool isEasdkRequiredPre = Easdk_is_required()
		bool isEasdkLoggedInPre = Easdk_is_loggedin()
		PrintLaunchDebugVal( "isEasdkRequiredPre", isEasdkRequiredPre )
		PrintLaunchDebugVal( "isEasdkLoggedInPre", isEasdkLoggedInPre )
		if ( isEasdkRequiredPre && !isEasdkLoggedInPre )
		{
			thread NucleusLogin( autoContinue ) 
			return
		}
	}
#endif                
...
#if CONSOLE_PROG
void function NucleusLogin(bool autoContinue )
{
	...
	if ( !Easdk_is_loggedin() )
	{
		...
		WaitFrame();
		if( !Easdk_is_logging_in() && !Easdk_is_logging_in() )
			Easdk_login()
#if SPINNER_DEBUG_INFO
