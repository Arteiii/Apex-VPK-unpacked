...
void function UpdateSurvivalGroundList( SurvivalGroundListUpdateParams params )
{
	...
	foreach ( entity lootEnt in params.currentLootEnts )
	{
		...
		foreach ( string entryDataKey, DeathBoxEntryData DA in fileLevel.deathBoxEntryDataByKey )
			printf( "Debug death box item 1: " + entryDataKey )
	...
	foreach ( DeathBoxEntryData entryData, void _ in entriesToUpdateSet )
	{
		...
		if ( shouldBeVisible )
		{
			if ( item == null )
			{
				...
				item = newItem
				printt("Item key " + entryData.key)
				printt("Item null case reached")
