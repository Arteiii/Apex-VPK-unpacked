...
void function ConsumableStickersPanel_Update( var panel )
{
	...
	if ( IsPanelActive( panel ) && pd.consumable != -1 )
	{
		...
		foreach ( int stickerItemIdx, ItemFlavor stickerItem in pd.stickerItemList )
		{
			...
			CustomizeButton_UpdateAndMarkForUpdating( button, entries, stickerItem, PreviewConsumableSticker, CanEquipCanBuyCharacterItemCheck )
		...
		ItemFlavor equippedStickerItem = LoadoutSlot_GetItemFlavor( ToEHI( GetLocalClientPlayer() ), entries[0] )
...
void function PreviewConsumableSticker( ItemFlavor stickerItem )
{
	if ( file.previewOnObject )
	{
		...
		RunClientScript( "UIToClient_PreviewAppliedStickerV1", ItemFlavor_GetGUID( stickerItem ), file.currentConsumable )
	}
	else
	{
		UI_SetPresentationType( ePresentationType.UNAPPLIED_STICKER )
		RunClientScript( "UIToClient_PreviewUnappliedSticker", ItemFlavor_GetGUID( stickerItem ) )
	}
